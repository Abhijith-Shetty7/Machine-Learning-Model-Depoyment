...
X_train[config.NUMERICAL_TO_IMPUTE] = pf.impute_na(X_train, config.NUMERICAL_TO_IMPUTE, replacement=config.LOTFRONTAGE_MODE)


# capture elapsed time
X_train[config.YEAR_VARIABLE] = pf.elapsed_years(X_train, config.YEAR_VARIABLE, ref_var='YrSold')
...
for var in config.CATEGORICAL_ENCODE:
    X_train[var] = pf.encode_categorical(X_train, var, config.ENCODING_MAPPINGS[var])
